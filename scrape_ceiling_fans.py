...
from ReviewSQL import CeilingFanWithLight, CeilingFan, Session
import numpy as np
...
def save_ceiling_fans_to_db(df):
    ...
    for index, row in df.iterrows():
        # 检查记录是否已经存在
        existing_record = session.query(CeilingFan).filter_by(
            itemId=row['itemId'],
...
def scrape_data():
    with open('headers_LCF.json', 'r') as file:
        headers = json.load(file)

    with open('payload_LCF.json', 'r') as file:
        payload = json.load(file)
    ...
    df.to_excel(f'ceiling_fans_{current_time}.xlsx')
