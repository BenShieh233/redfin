...
from ReviewSQL import Review_LCF, Session
import pandas as pd
from scrape_ceiling_fans import scrape_data

# 检查 itemId 是否存在于数据库中
def check_item_exists(session, itemId, current_url):
    return session.query(Review_LCF).filter_by(itemId=itemId, current_url=current_url).first()
